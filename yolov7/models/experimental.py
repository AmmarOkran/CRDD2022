...
class Ensemble(nn.ModuleList):
    ...
    def forward(self, x, augment=False):
        ...
        # y1 = torch.stack(y).max(0)[0]  # max ensemble
        # y2 = torch.stack(y).median(0)[0] # median ensemble
        # y3 = torch.stack(y).mean(0)  # mean ensemble

        # y = torch.cat((y1, y2, y3), 1)#/w_sum  # nms ensemble
        y = torch.cat(y, 1)#/w_sum  # nms ensemble
        return y, None  # inference, train output
    
def equalized(data):
    res = []
    for i, d in enumerate(data):
        max_len = max([(x.shape[1], i) for i, x in enumerate(data)])
        if d.shape[1] < max_len[0]:
            pad_tens = data[max_len[1]][:, d.shape[1]: d.shape[1] + (max_len[0] - d.shape[1]),:] #torch.zeros((d.shape[0], max_len[0] - d.shape[1], d.shape[2])).cuda()
            d = torch.cat((d, pad_tens), 1)
        res.append(d)
    return res
